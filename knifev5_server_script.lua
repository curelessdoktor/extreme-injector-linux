-- remade by curelessdoktor optimising stuff, have fun skids. (server – full knife logic, replicates to everyone)
-- Put this in a Script in ServerScriptService. Create RemoteEvent "KnifeRemote" in ReplicatedStorage.
...
local function getState(player)
	if not state[player] then
		state[player] = { equipped = false, working = false, grabbed = nil, grabweld = nil, handle = nil, hweld = nil, grabbing = false }
	end
	return state[player]
end
...
local function setupCharacter(player)
	...
	s.working = false
	s.grabbed = nil
	if s.grabweld then pcall(function() s.grabweld:Destroy() end) end
	s.grabweld = nil
end
...
Players.PlayerAdded:Connect(function(player)
	...
	if player.Character then setupCharacter(player) end
end)
...
KnifeRemote.OnServerEvent:Connect(function(player, action, targetChar)
	local s = getState(player)
	local char = player.Character
	if not char or not s.handle or s.handle.Parent ~= char then return end
	local handle, hweld = s.handle, s.hweld

	local function getrid()
		if s.grabbed then
			unstun(s.grabbed)
			s.grabbed = nil
			if s.grabweld then s.grabweld:Destroy() s.grabweld = nil end
		end
		for _, ree in ipairs(handle:GetChildren()) do
			if ree:IsA('BasePart') then ree:Destroy()
			else ree:Remove() end
		end
	end

	local function doEquip()
		if s.equipped or s.working then return end
		s.equipped = true
		s.working = true
		...
		local function mkp(isWedge, name, size, color, mat, shape)
			...
			p.Size = size
			p.Anchored = false
			p.CanCollide = (name == "blade")
			if not isWedge then p.Friction = 0.3 end
			p.Color = color
			if shape then p.Shape = shape end
			for _, surf in ipairs({ "Top", "Bottom", "Left", "Right", "Front", "Back" }) do p[surf .. "Surface"] = Enum.SurfaceType.SmoothNoOutlines end
			unanchorPart(p)
			return p
		end
		...
		obj3.Name = "blade"
		obj3.Parent = handle
		for _, surf in ipairs({ "Top", "Bottom", "Left", "Right", "Front", "Back" }) do obj3[surf .. "Surface"] = Enum.SurfaceType.SmoothNoOutlines end
		obj3.Material = Enum.Material.Metal
		obj3.Size = Vector3.new(0.1, 0, 0.23)
		obj3.Anchored = false
		obj3.Color = Color3.fromRGB(99, 95, 98)
		obj3.CanCollide = true
		unanchorPart(obj3)
		...
		local function connectBladeTouched(bladePart)
			bladePart.Touched:Connect(function(hit)
				if not hit or not hit.Parent then return end
				if not s.grabbing or s.grabbed then return end
				...
				gw.Part0 = char.Torso
				pcall(function() gw.Part1 = otherChar.Torso end)
				pcall(function() gw.Part1 = otherChar.UpperTorso end)
				gw.C0 = CFrame.new(-0.45, 0, -1)
				s.grabweld = gw
				KnifeRemote:FireClient(player, "Grabbed")
			end)
		end
		connectBladeTouched(obj4)
		connectBladeTouched(obj5)
		connectBladeTouched(obj3)
		task.spawn(function()
			...
			task.delay(0.5, function() pe.Enabled = false Debris:AddItem(pe, 0.6) end)
			for step = 1, 25 do
				task.wait(0.02)
		...
		s.working = false
		KnifeRemote:FireClient(player, "Equipped")
	end

	local function doUnequip()
		if not s.equipped or s.working then return end
		getrid()
		s.equipped = false
		s.working = true
		local rs = char.Torso:FindFirstChild("Right Shoulder")
		...
		restoreArms(char)
		s.working = false
		KnifeRemote:FireClient(player, "Unequipped")
	end

	local function doGrab()
		if s.grabbed or not s.equipped or s.working then return end
		s.working = true
		task.spawn(function()
			...
			local grabDeadline = tick() + 0.4
			while tick() < grabDeadline and not s.grabbed do
				for _, desc in ipairs(handle:GetDescendants()) do
					if desc:IsA("BasePart") and desc.Name == "blade" then
						for _, touching in ipairs(desc:GetTouchingParts()) do
							if not touching.Parent then
							elseif touching.Parent == char then
							else
								local otherChar = touching.Parent
								local hum = otherChar:FindFirstChildOfClass("Humanoid")
								if hum and hum.Health > 0 then
									local otherPlayer = Players:GetPlayerFromCharacter(otherChar)
	...
	local function doKill()
		if not s.grabbed or s.working then return end
		local grabbed = s.grabbed
		s.working = true
		task.spawn(function()
			local rweld = char["Right Arm"]:FindFirstChild("Weld")
			local lweld = char["Left Arm"]:FindFirstChild("Weld")
			local tweld = makeWeld(char.HumanoidRootPart, char.HumanoidRootPart, char.Torso)
			...
			task.wait(0.2)
			if s.grabweld then s.grabweld:Remove() s.grabweld = nil end
			ts:Play()
			...
			s.grabbed = nil
			s.working = false
			KnifeRemote:FireClient(player, "Killed")
		end)
	end
