--[[
	Grab Knife v2 — Optimized for Roblox 2026
	Runs when the player joins and on every respawn (CharacterAdded).
	Uses Tool (replaces HopperBin), modern APIs, R6/R15 support.
]]
...
local function buildBricks(character)
	...
	-- Right hold (grab hitbox - larger = easier to grab)
	...
	-- Left hold (grab hitbox - larger = easier to grab)
...
local function setupTool(player)
	...
	local function selectTool()
		...
		-- Key bindings
		if state.keyConnection then
			state.keyConnection:Disconnect()
		end
		state.keyConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
			if gameProcessed then return end
			local key = input.KeyCode.Name:lower()
			if key == "q" then
				state.mode = "drop"
				inform(player:WaitForChild("PlayerGui"), "Mode: Drop", 2)
			elseif key == "e" then
				state.mode = "throw"
				inform(player:WaitForChild("PlayerGui"), "Mode: Throw", 2)
			elseif key == "f" then
				state.mode = "kill"
				inform(player:WaitForChild("PlayerGui"), "Mode: Kill", 2)
			end
		end)
	end
...
-- Entry: run when player's character loads (join + respawn)
